...
- **Design System**: Check if a design system exists for the project via
  ...
  **stitch-manage-design-system** skill first before generating screens.
...
handled by the stitch-manage-design-system skill at the project level).
...
> project level by the stitch-manage-design-system skill and will conflict if
...
When the user wants the design language itself upgraded before generation,
delegate to `stitch-taste-design` or `stitch-design-md` before finalizing the
prompt.
...
Delegate to the **stitch-upload-to-stitch** skill to upload the image to the project.
...
**stitch-manage-design-system** skill's `examples/metadata.json` for the format.
