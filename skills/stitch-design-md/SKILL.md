...
description: Analyze existing Stitch projects and synthesize a semantic design system into a native `.stitch/DESIGN.md` file for stitch-orchestrator.
allowed-tools:
  - "stitch*:*"
  - "Read"
  - "Write"
...
Use the bundled `stitch` MCP server from this plugin. Your goal is to analyze
existing Stitch assets and synthesize a semantic design system into
`.stitch/DESIGN.md`.
...
The `.stitch/DESIGN.md` file will serve as the source of truth for prompting
Stitch to generate new screens that align with the existing design language.
Also update `.stitch/metadata.json` when the project or representative screens
become clearer during analysis.

## Retrieval and Networking

To analyze a Stitch project, you must retrieve screen metadata and design assets using the bundled `stitch` MCP tools:

1. **Project lookup** (if Project ID is not provided):
   - Call `list_projects` with `filter: "view=owned"` to retrieve all user projects
   - Identify the target project by title or URL pattern
   - Extract the Project ID from the `name` field (e.g., `projects/13534454087919359824`)

2. **Screen lookup** (if Screen ID is not provided):
   - Call `list_screens` with the `projectId` (just the numeric ID, not the full path)
   - Review screen titles to identify the target screen (e.g., "Home", "Landing Page")
   - Extract the Screen ID from the screen's `name` field

3. **Metadata fetch**: 
   - Call `get_screen` with both `projectId` and `screenId` (both as numeric IDs only)
...
5. **Project metadata extraction**:
   - Call `get_project` with the project `name` (full path: `projects/{id}`) to get:
