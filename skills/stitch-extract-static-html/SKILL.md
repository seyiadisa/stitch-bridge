...
- Node.js with `puppeteer` available from this plugin's `pnpm install`
- A local Chrome/Chromium/Edge install, or a working Puppeteer-managed browser download
...
| `Cannot find module 'puppeteer'` | Run `pnpm install` in the plugin folder so the bundled dependency is available. |
| `Could not find Chrome` | Set `PUPPETEER_EXECUTABLE_PATH` or install Chrome/Edge locally so the snapshot script can launch a browser. |
